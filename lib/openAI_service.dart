...
import 'package:gpt/secret.dart';
...
class OpenAIservice {
  ...
  Future<String> isArtPromptAPI(String prompt) async {
    try {
      final res = await http.post(Uri.parse('https://api.openai.com/v1/chat/completions'),
        ...
        body: jsonEncode({
          ...
          "messages": [
            {
              "role": "user",
              "content": "Does this message want to generate as AI picture? $prompt . simply answer with a yes or no.",
            },
          ]
        }
        ),
      ...
      return 'An internal error eccurred';
    } catch (e) {
      return e.toString();
    }
  }
  ...
  Future<String> dellEAPI(String prompt) async {
    ...
    try {
      ...
      print(res.body);
      if(res.statusCode == 200){
        String imageUrl = jsonDecode(res.body)['data'][0]['message']['url'];
